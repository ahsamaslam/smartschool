...
function ClassesView({ onSelectClass }) {
  ...
  return (
    <div>
            ...
            >
              <div className="flex flex-col">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setShowDeleteModal(cls); }}
                    className="p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100 flex-shrink-0"
                  >
                    <TrashIcon className="h-4 w-4" />
                  </button>
                ...
                <div className="flex gap-4 text-xs text-gray-500 pt-3 border-t border-gray-100">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>{cls.book_count || 0} books</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* Delete Class Modal */}
      <Modal isOpen={!!showDeleteModal} onClose={() => !deleting && setShowDeleteModal(null)} title="Delete Class">
        {showDeleteModal && (
          <div className="space-y-4">
            <div className="bg-red-50 rounded-xl p-4 text-sm text-red-700">
              <p className="font-semibold mb-1">This cannot be undone.</p>
              <p>Deleting <strong>"{showDeleteModal.name}"</strong> will permanently remove it along with all its subjects and books.</p>
            </div>
            <div className="flex gap-3">
              <Button variant="secondary" onClick={() => setShowDeleteModal(null)} disabled={deleting} fullWidth>Cancel</Button>
              <Button variant="danger" onClick={handleDeleteClass} loading={deleting} fullWidth>Delete Class</Button>
            </div>
          </div>
        )}
      </Modal>
      ...
      {/* Add Class Modal */}
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Add Class">
        <form onSubmit={handleAddClass} className="space-y-4">
          <div>
            ...
            <select
              value={classForm.preset}
              onChange={(e) => setClassForm((f) => ({ ...f, preset: e.target.value, custom: "" }))}
              className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              required
            >
          ...
          {classForm.preset === "__custom__" && (
            <div>
              ...
              <input
                type="text"
                value={classForm.custom}
                onChange={(e) => setClassForm((f) => ({ ...f, custom: e.target.value }))}
                placeholder="e.g., Pre-School, Grade 13…"
                className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
                required
              />
            </div>
