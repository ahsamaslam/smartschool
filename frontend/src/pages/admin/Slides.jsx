...
import { SLIDE_TEMPLATES, SLIDE_LAYOUTS, SLIDE_ANIMATIONS } from "../../data/slideTemplates";
import {
  SparklesIcon,
  PlusIcon,
  TrashIcon,
...
export default function AdminSlides() {
  ...
  const motionClass =
    SLIDE_ANIMATIONS.find((a) => a.id === (currentSlide?.animation || animation))?.css || "";
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const withIds = newSlides.map((s, i) => ({ ...s, id: `s${Date.now()}_${i}` }));
      setSlides(withIds);
      setCurrentIndex(0);
      ...
      toast.error(err?.response?.data?.detail || err?.message || "Generation failed", { id: tid });
    } finally {
      setGenerating(false);
    }
  };
  ...
  const updateTitleAt = useCallback((slideIdx, val) => {
    setSlides((prev) => prev.map((s, i) => (i === slideIdx ? { ...s, title: val } : s)));
  }, []);
  ...
  const updateSlideContentLines = useCallback((slideIdx, text) => {
    ...
    setSlides((prev) => prev.map((s, i) => (i === slideIdx ? { ...s, content: lines.length ? lines : [""] } : s)));
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (deck.length > 0) {
          setSlides(deck.map((s, idx) => ({ ...s, id: s.id || `s_import_${Date.now()}_${idx}` })));
          setCurrentIndex(0);
        }
        if (row.slide_theme) {
  ...
  const updateLayout = (layoutId) => {
    setSlides((prev) =>
      prev.map((s, i) => {
        ...
        else if (layoutId === "title-bullets" || layoutId === "two-column") animationNext = "stagger";
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      if (e.key === "ArrowRight" || e.key === "ArrowDown") setPreviewIndex((p) => Math.min(p + 1, slides.length - 1));
      if (e.key === "ArrowLeft" || e.key === "ArrowUp") setPreviewIndex((p) => Math.max(p - 1, 0));
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      if (e.key === "ArrowRight" || e.key === "ArrowDown") setCurrentIndex((p) => Math.min(p + 1, slides.length - 1));
  ...
  if (previewMode && slides.length > 0) {
    const pAnim = SLIDE_ANIMATIONS.find((a) => a.id === slides[previewIndex]?.animation)?.css || "";
    return (
      <div className="fixed inset-0 z-50 bg-black flex flex-col">
        <div className="flex-1 flex items-center justify-center p-4">
          <div className={`w-full max-w-5xl ${pAnim}`} style={{ aspectRatio: "16/9" }}>
            <SlideRenderer slide={slides[previewIndex]} template={template} slideIndex={previewIndex} />
          </div>
        </div>
        <div className="flex items-center justify-between px-6 py-3 bg-black/80 border-t border-white/10">
          <button type="button" onClick={() => setPreviewMode(false)} className="text-white/60 hover:text-white text-sm flex items-center gap-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              type="button"
              onClick={() => setPreviewIndex((p) => Math.min(slides.length - 1, p + 1))}
  ...
  if (deckEditorOpen && slides.length > 0 && currentSlide) {
    const dAnim =
      SLIDE_ANIMATIONS.find((a) => a.id === (slides[currentIndex]?.animation || animation))?.css || "";
    return (
      <div className="fixed inset-0 z-[60] bg-[#dfe3ea] flex flex-col">
        <header className="h-11 flex-shrink-0 flex items-center justify-between px-4 bg-white border-b border-gray-200 shadow-sm gap-4">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-semibold text-gray-800 truncate">Slide editor</span>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            >
              Done <span className="text-white/70 text-xs hidden sm:inline">Esc</span>
            </button>
          </div>
        </header>
        <main className="flex-1 flex flex-col items-center justify-center px-6 py-6 min-h-0 overflow-auto">
          ...
          >
            <SlideRenderer
              ...
              onUpdateContent={(bi, val) => updateContentAt(currentIndex, bi, val)}
            />
          </div>
          <p className="mt-4 text-xs text-gray-500 text-center max-w-xl">
            Click text to edit • Arrow keys switch slides • Double-click thumbnails or canvas in the studio view to open this again
          </p>
        </main>
      </div>
    );
        ...
        {!leftPanelCollapsed ? (
          <>
            <div className="px-4 py-4 border-b border-gray-100 flex items-center gap-2">
              <button type="button" onClick={() => navigate(-1)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 transition-colors flex-shrink-0">
                <ArrowLeftIcon className="h-4 w-4" />
              </button>
              <div className="min-w-0 flex-1">
                <h1 className="text-sm font-bold text-gray-900 leading-tight">AI Slide Studio</h1>
                <p className="text-xs text-gray-400">Gamma · Canva–style authoring</p>
              </div>
              <button
                type="button"
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Content / notes</label>
            <textarea
              value={contentInput}
              ...
              {[
                ...
                <button
                  ...
                  className={`py-2 rounded-xl text-sm font-medium border transition-all ${
                    audience === val ? "bg-indigo-600 text-white border-indigo-600 shadow-sm" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300"
                  }`}
                >
                  {label}
                </button>
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button
                  ...
                  onClick={() => setTone(val)}
                  className={`py-2 rounded-xl text-sm font-medium border transition-all ${
                    tone === val ? "bg-indigo-600 text-white border-indigo-600 shadow-sm" : "bg-white text-gray-600 border-gray-200 hover:border-indigo-300"
                  }`}
                >
                  {label}
                </button>
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">
              Slides <span className="text-indigo-600 font-bold">{slideCount}</span>
            </label>
            <input type="range" min={4} max={20} step={1} value={slideCount} onChange={(e) => setSlideCount(Number(e.target.value))} className="w-full accent-indigo-600" />
          ...
          <label className="flex items-center gap-2 text-xs text-gray-600 cursor-pointer select-none">
            <input type="checkbox" checked={persistToDb} onChange={(e) => setPersistToDb(e.target.checked)} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
            ...
            <>
              <button
                ...
                className={`w-full py-2 rounded-xl border text-xs font-semibold transition-all flex items-center justify-center gap-2 ${
                  bulkEditorOpen ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-gray-200 bg-white text-gray-700 hover:border-indigo-200"
                }`}
              >
                <Squares2X2Icon className="h-4 w-4" /> {bulkEditorOpen ? "Hide" : "Show"} bulk text editor
              </button>

              {bulkEditorOpen && (
                <div className="rounded-xl border border-gray-100 bg-gray-50/90 p-2 max-h-64 overflow-y-auto space-y-2">
                  {slides.map((sl, idx) => (
                    <details key={sl.id || idx} className="group rounded-lg bg-white border border-gray-100 px-2 py-1.5">
                      <summary className="text-[11px] font-bold text-gray-600 cursor-pointer list-none flex justify-between items-center [&::-webkit-details-marker]:hidden">
                        <span>
                          Slide {idx + 1}
                          <span className="font-normal text-gray-400 ml-1 truncate max-w-[8rem] inline-block align-bottom">{sl.title}</span>
                        </span>
                      ...
                      <div className="pt-2 space-y-2 pb-1">
                        ...
                        />
                        <label className="block text-[10px] uppercase font-semibold text-gray-400">Lines (bullet / step / column items)</label>
                        <textarea
                          value={(sl.content || []).join("\n")}
                          onChange={(e) => updateSlideContentLines(idx, e.target.value)}
                          rows={Math.min(8, Math.max(3, (sl.content || []).length + 2))}
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden min-w-0">
        <div className="h-12 flex-shrink-0 bg-white/90 backdrop-blur border-b border-gray-200/80 flex items-center justify-between px-4 gap-3 shadow-sm">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="text-sm font-semibold text-gray-800 truncate">{topicInput || "Untitled deck"}</span>
            {slides.length > 0 && <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full flex-shrink-0">{slides.length}</span>}
          </div>
          <div className="flex items-center gap-2 flex-shrink-0">
            {slides.length > 0 && (
              <>
            ...
            <>
              <div
                className={`flex-shrink-0 bg-[#dfe3ea] border-r border-gray-200/90 flex flex-col items-center gap-2 py-3 overflow-y-auto overflow-x-hidden transition-[width,max-width] duration-200 ease-out ${
                  filmstripCollapsed ? "w-0 max-w-0 border-transparent p-0" : "w-36 max-w-36 px-2"
                }`}
              >
                {!filmstripCollapsed &&
                  slides.map((slide, i) => (
                    <div key={slide.id} className="flex flex-col items-center gap-0.5 w-full group/thumb">
                  <SlideThumbnail
                    slide={slide}
                    template={template}
                    index={i}
                    ...
                    onDrop={() => handleDrop(i)}
                  />
                  <div className={`flex gap-1 opacity-100 ${i === currentIndex ? "" : "md:opacity-0 md:group-hover/thumb:opacity-100"} transition-opacity`}>
                    <button type="button" onClick={(e) => { e.stopPropagation(); duplicateSlide(i); }} title="Duplicate" className="p-0.5 text-gray-500 hover:text-indigo-600">
                      <DocumentDuplicateIcon className="h-3 w-3" />
                    </button>
                    <button type="button" onClick={(e) => { e.stopPropagation(); removeSlide(i); }} title="Delete" className="p-0.5 text-gray-500 hover:text-red-500">
              ...
              <div className="w-9 flex-shrink-0 flex flex-col items-center pt-3 bg-[#c8cedc] border-r border-gray-200/70">
                <button
                  ...
                  title={filmstripCollapsed ? "Show slide thumbnails" : "Hide thumbnails for more canvas space"}
                >
                  {filmstripCollapsed ? (
          ...
          <div className="flex-1 overflow-auto bg-[#cdd2df] flex items-start justify-center p-6 min-w-0">
            {slides.length === 0 ? (
              <div className="text-center max-w-md mt-24">
                ...
                <h2 className="text-2xl font-bold text-gray-800 mb-2">Design-grade decks</h2>
                <p className="text-gray-500 text-sm leading-relaxed">
                  Describe your lesson on the left. We structure layouts, split screens, steps, and motion — then you polish like a pro editor.
                </p>
              </div>
              ...
              <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-6 pb-10">
                {slides.map((slide, i) => {
                  const cls = SLIDE_ANIMATIONS.find((a) => a.id === slide.animation)?.css || "";
                  return (
                    <div
                      tabIndex={0}
                      key={slide.id}
                      ...
                      className={`text-left rounded-2xl bg-white shadow-lg ring-2 transition hover:shadow-xl overflow-hidden cursor-pointer outline-none focus-visible:ring-indigo-500 focus-visible:ring-offset-2 ${
                        i === currentIndex ? "ring-indigo-500" : "ring-transparent"
                      }`}
                    >
                      <div className={`${cls}`} style={{ aspectRatio: "16/9" }}>
                        <SlideRenderer
                          ...
                          onUpdateContent={(bi, val) => updateContentAt(i, bi, val)}
                        />
                      </div>
                      <div className="px-3 py-2 flex items-center justify-between bg-gray-50 border-t border-gray-100 pointer-events-none">
                        ...
                        <span className="text-[10px] uppercase tracking-wide text-indigo-600 font-bold">{slide.layout}</span>
                      </div>
                    </div>
                  );
                })}
        ...
        {rightPanelCollapsed ? (
          ...
          <>
            <div className="flex items-center justify-between px-2 py-2 border-b border-gray-200 bg-gray-50/80">
              <span className="text-[10px] font-bold text-gray-500 uppercase tracking-wide pl-1">Design</span>
              <button
                type="button"
              ...
              {[
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 text-[11px] font-semibold uppercase tracking-wide transition-colors border-b-2 ${
                    rightTab === key ? "border-indigo-600 text-indigo-700 bg-white" : "border-transparent text-gray-500 hover:text-gray-700"
                  }`}
                >
          ...
          {rightTab === "templates" && (
            <div className="space-y-2">
              <p className="text-xs text-gray-400 mb-2">{SLIDE_TEMPLATES.length} premium looks — swap instantly</p>
              {Object.entries(
                    ...
                    {templates.map((t) => (
                      <button
                        ...
                        className={`relative rounded-xl overflow-hidden border-2 transition-all text-left ${
                          template.id === t.id ? "border-indigo-500 shadow-md scale-[1.02]" : "border-transparent hover:border-gray-300"
                        }`}
                      >
                        <div
                          className="h-14 w-full flex"
                          style={{ background: typeof t.css.slideBackground === "string" ? t.css.slideBackground : "#fff" }}
                        >
                          <div className="flex-1 flex flex-col justify-center px-2 gap-1">
                            <div className="h-1.5 rounded-full w-10" style={{ background: t.css.titleColor, opacity: 0.9 }} />
                            <div className="h-1 rounded-full w-16" style={{ background: t.css.bodyColor, opacity: 0.55 }} />
                            <div className="h-1 rounded-full w-12" style={{ background: t.css.accentColor, opacity: 0.5 }} />
                          </div>
                          <div className="w-1.5 self-stretch" style={{ background: t.css.accentColor }} />
                        </div>
                        <div className={`px-2 py-1.5 text-center ${template.id === t.id ? "bg-indigo-50" : "bg-gray-50"}`}>
                          <span className="text-[10px] font-semibold text-gray-700 leading-tight block">{t.name}</span>
                        </div>
                        {template.id === t.id && (
          ...
          {rightTab === "layouts" && (
            <div className="space-y-2">
              <p className="text-xs text-gray-400 mb-2">Current slide structure</p>
              {SLIDE_LAYOUTS.map((layout) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl border-2 text-left transition-all disabled:opacity-40 ${
                    currentSlide?.layout === layout.id ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:border-indigo-300 bg-white"
                  }`}
                >
                  <span className="text-lg w-8 text-center font-bold" style={{ color: currentSlide?.layout === layout.id ? "#4f46e5" : "#9ca3af" }}>
                    {layout.icon}
                  </span>
                  <div>
                  ...
                  {currentSlide?.layout === layout.id && <CheckIcon className="h-4 w-4 text-indigo-600 ml-auto flex-shrink-0" />}
                </button>
              ))}
            </div>
          )}
          ...
          {rightTab === "animations" && (
            <div className="space-y-2">
              <p className="text-xs text-gray-400 mb-2">Entrance motion (present mode + canvas)</p>
              {SLIDE_ANIMATIONS.map((anim) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-3 py-3 rounded-xl border-2 text-left transition-all ${
                    (currentSlide?.animation || animation) === anim.id ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:border-indigo-300 bg-white"
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <BoltIcon className="h-4 w-4 text-gray-400" />
                    <span className="text-sm font-medium text-gray-800">{anim.name}</span>
                  </div>
                  {(currentSlide?.animation || animation) === anim.id && <CheckIcon className="h-4 w-4 text-indigo-600" />}
                </button>
              ))}
            </div>
          )}
            ...
            {currentSlide && (
              <div className="border-t border-gray-100 px-4 py-3 bg-gray-50 flex-shrink-0">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Slide {currentIndex + 1}</p>
                <p className="text-xs text-gray-800 truncate font-semibold mt-0.5">{currentSlide.title}</p>
                <div className="flex flex-wrap items-center gap-2 mt-2">
                  <span className="text-[10px] bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-full font-semibold">{currentSlide.layout}</span>
                  <span className="text-[10px] bg-gray-200/80 text-gray-700 px-2 py-0.5 rounded-full font-semibold">{currentSlide.animation || animation}</span>
                  <span className="text-[10px] bg-white text-gray-500 px-2 py-0.5 rounded-full border border-gray-200">{template.name}</span>
                </div>
              </div>
            )}
          </>
