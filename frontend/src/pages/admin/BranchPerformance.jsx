...
export default function BranchPerformance() {
  ...
  useEffect(() => {
    ...
    adminService
      ...
      .catch(() => {
        const stateClasses = location.state?.classes;
        if (stateClasses) {
          setClasses([...stateClasses].sort((a, b) => parseFloat(b.avg_cvi) - parseFloat(a.avg_cvi)));
        }
        if (location.state?.branchName) setBranchName(location.state.branchName);
      })
  ...
  const teachers = Object.values(teacherMap)
    .map((t) => ({
      ...t,
      avg_cvi: t.cvi_count > 0 ? t.cvi_sum / t.cvi_count : 0,
      teacher_grade: t.grades.length > 0 ? t.grades[0] : null,
    }))
  ...
  if (loading) {
    return <div className="p-6 text-center py-20 text-gray-400 text-sm">Loading…</div>;
  }
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto pb-24">
      ...
      <div className="flex items-center gap-3 mb-1">
        <button
          type="button"
          onClick={() => navigate(`/admin/schools/${schoolId}/performance`)}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {teachers.map((t, i) => (
                  <tr
                    key={t.teacher_name}
                    className={t.avg_cvi > 0 && t.avg_cvi < 60 ? "bg-red-50/50" : t.avg_cvi >= 85 ? "bg-blue-50/30" : ""}
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      <GradeChip grade={t.teacher_grade} />
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              <tbody className="divide-y divide-gray-100">
                {classes.map((cls) => {
                  ...
                  return (
                    <tr
                      key={cls.class_id}
                      className={hasCvi && cvi < 60 ? "bg-red-50/30" : hasCvi && cvi >= 85 ? "bg-blue-50/20" : "hover:bg-gray-50"}
                    >
                      ...
                      <td className="px-4 py-3 text-center hidden md:table-cell">
                        {cls.teacher_grade ? <GradeChip grade={cls.teacher_grade} /> : <span className="text-xs text-gray-400">—</span>}
                      </td>
                    </tr>
                  );
                })}
